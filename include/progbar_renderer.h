...
struct BaseRendererConfiguration {
  enum RenderAlignment { Left, Right, Center } alignment = Center;
};
...
public:
  ...
  RendererStateType get_state() { return m_state; }
  void set_state(RendererStateType st) { this->m_state = st; }
...
public:
  int render_height() override { return 3; }
  ...
  std::string render(size_t max_steps, size_t current_steps) override {
    ...
    if (m_state.alignment == BaseRendererConfiguration::RenderAlignment::Left) {
      padding_count = 0;
    } else if (m_state.alignment ==
               BaseRendererConfiguration::RenderAlignment::Right) {
      padding_count = term_width - render_width;
    } else if (m_state.alignment ==
               BaseRendererConfiguration::RenderAlignment::Center) {
    ...
    case LineRendererState::ProgressionDirection::Inwards: {
      ...
      std::string progress_line =
          ansi::FormatBuilder()
              ...
              .fg(m_state.active_color)
              .write(
                  std::string(render_actives_right, m_state.line_unit_active))
              .str(true);
      BAR += progress_line;
    } break;
    case LineRendererState::ProgressionDirection::Outwards: {
      ...
      std::string progress_line =
          ansi::FormatBuilder()
              .fg(m_state.inactive_color)
              .bold()
              .write(std::string(render_inactives_left,
                                 m_state.line_unit_inactive))
              .fg(m_state.active_color)
              .write(std::string(active_count, m_state.line_unit_active))
              .fg(m_state.inactive_color)
              .write(std::string(render_inactives_right,
                                 m_state.line_unit_inactive))
              .str(true);
      BAR += progress_line;
    } break;
...
private:
  ...
  void put_centrally(std::string &txt, size_t max_width, std::string text,

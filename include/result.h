...
private:
    ...
    */
    T value_or_throw(){
        if(m_variant == Good)return std::move(m_good);
        else throw std::move(m_bad);
    }
    /**
     * Returns the value contained within the Result if it is Good
    ...
    */
    T value_or_panic(){
        if(m_variant == Good)return std::move(m_good);
        else panic("Result contained an Error, but was accessed via value_or_panic()");
    }
    ...
    */
    ...
    std::optional<E> error(){
        if(m_variant == Bad)return std::move(m_bad);
        else return {};
    }
};
